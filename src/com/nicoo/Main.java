...
public class Main {
    // static objects and variables, so that other classes can access them.
    static Scanner read = new Scanner(System.in);
    static Buyer buyer = new Buyer();
    static String shoeName;
    static String customerName;
    public static void main(String[] args) {
        System.out.print("Is this shoe available?: " );
        buyer.asking();
        // seller object
        ...
        seller.isAvailable();
        seller.available();
        seller.sPrice(shoeName);

        System.out.println("How may I address you?: " );
        buyer.name();

        buyer.buy(shoeName);
        seller.invoice(shoeName);
        buyer.pay();
    }
// All of these static classes are connected to each other by inheritance.
    ...
    static class Seller extends Billing{
        ...
        void isAvailable() {
            isAvail = setShoeList().contains(shoeName.toUpperCase());
        }
        void available(){
            if (isAvail) {
                System.out.println( "Yes "+ shoeName.toUpperCase(Locale.ROOT)+" is available");
            }else{
                System.out.println("Sorry, "+ shoeName.toUpperCase(Locale.ROOT) + " is not available yet");
                System.exit(0);
            }
        }
    }
    ...
    static class PriceList extends ShoeShelf {
        ...
        // This method prints out the price of a shoe,
         void sPrice(String shoe){
             System.out.println(    );
             System.out.println("The price of "+ shoe.toUpperCase(Locale.ROOT) +" is P"+shoePrice[(setShoeList().indexOf(shoe.toUpperCase(Locale.ROOT)))]);
         }
    }
    // Billing class, contains a method that prints out the invoice with the billing info.
        ...
        void invoice(String item){
            String[] inv = new String[]{" ", "NAME                           " + customerName.toUpperCase(Locale.ROOT) , " ", "ITEM                           "+ item.toUpperCase(Locale.ROOT)," ",  " "," ", "TOTAL                          P    "+ (shoePrice[setShoeList().indexOf(item.toUpperCase(Locale.ROOT))]), " ", " "};
            System.out.println();
            System.out.println(" ============================================");
